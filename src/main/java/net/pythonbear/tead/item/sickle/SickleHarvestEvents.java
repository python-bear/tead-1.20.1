...
public final class SickleHarvestEvents {
    /**
     * Priority phase for listeners that will be called first.
     */
    public static final Identifier PRIORITY_PHASE = new Identifier(Tead.MOD_ID, "priority");
    /**
     * Deferred phase for listeners that will be called last.
     */
    public static final Identifier DEFERRED_PHASE = new Identifier(Tead.MOD_ID, "deferred");
    ...
    public interface HarvestCheck {
         ...
         */
        boolean check(World world, BlockState crop, BlockPos pos, PlayerEntity player, Hand hand, boolean first,
                      HarvestCheckEvent event);
    }
    ...
    public interface BeforeHarvest {
         ...
         * @param hand {@link Hand} used to harvest.
         */
        void beforeHarvest(ServerWorld world, BlockState crop, BlockPos pos, Direction face, BlockHitResult result,
                           ServerPlayerEntity player, Hand hand, boolean first);
    }
    ...
    public interface HarvestDrops {
         ...
         */
        List<ItemStack> getDrops(ServerWorld world, BlockState crop, BlockPos pos, Direction face,
                                 BlockHitResult result, ServerPlayerEntity player, Hand hand, boolean first,
    ...
    public interface AfterHarvest {
         ...
         * @param first whether the current crop is the actual right-clicked crop (the one also at the center of the harvest area).
         */
        void afterHarvest(ServerWorld world, BlockState crop, BlockPos pos, Direction face, BlockHitResult result,
                          ServerPlayerEntity player, Hand hand, boolean first);
    }


    private static abstract class SickleHarvestEvent {
        /**
        ...
        public void setCanceled(boolean cancel) {
            if (!isCancelable()) {
                throw new UnsupportedOperationException("Attempted to call setCanceled() on a non-cancelable event " +
     ...
     */
    public static class HarvestCheckEvent extends SickleHarvestEvent {
        /**
     ...
     * Event fired when calculating the drops resulting from right-click harvesting.
     */
    public static class HarvestDropsEvent extends SickleHarvestEvent {
        /**
         * List of drops.
         */
         ...
         * @param hand {@link Hand} used to harvest.
         */
        public HarvestDropsEvent(ServerWorld world, BlockState crop, BlockPos pos, ServerPlayerEntity player,
                                 Hand hand) {
         ...
         */
        private List<ItemStack> initDrops(ServerWorld world, BlockState crop, BlockPos pos, ServerPlayerEntity player,
                                          Hand hand) {
            List<ItemStack> drops = Block.getDroppedStacks(crop, world, pos,
                    crop.hasBlockEntity() ? world.getBlockEntity(pos) : null, player, player.getStackInHand(hand));
