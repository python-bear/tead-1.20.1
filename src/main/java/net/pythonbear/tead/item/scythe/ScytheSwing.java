...
public class ScytheSwing {
    ...
    private static ActionResult handleAttackBlock(PlayerEntity playerEntity, World world, Hand hand, BlockPos blockPos,
                                                  Direction direction) {
        if (world.isClient) {
    ...
    private static boolean breakBlocksInRadius(PlayerEntity playerEntity, World world, Hand hand, BlockPos blockPos,
                                               BlockState targetedBlock, ItemStack stack) {
        ...
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
                didWork |= tryBreakBlock(playerEntity, world, hand, blockPos.add(x, 0, z), isTargetingCrop,
                        isTargetingNetherWart, isTargetingFullyGrown, stack);
            }
        }
    ...
    private static boolean isValidInitialTarget(BlockState targetedBlock) {
        return targetedBlock.isIn(BlockTags.CROPS) || targetedBlock.isOf(Blocks.NETHER_WART) ||
                targetedBlock.isIn(TeadTags.Blocks.SCYTHE_BLOCKS);
    }
    ...
    private static boolean tryBreakBlock(PlayerEntity playerEntity, World world, Hand hand, BlockPos targetPos,
                                         boolean isTargetingCrop, boolean isTargetingNetherWart,
                                         boolean isTargetingFullyGrown, ItemStack stack) {
        ...
        if (world instanceof ServerWorld) {
            List<ItemStack> drops = Block.getDroppedStacks(targetState, (ServerWorld) world, targetPos,
                    world.getBlockEntity(targetPos), playerEntity, stack);
    ...
    private static boolean shouldBreakBlock(boolean isTargetingCrop, boolean isTargetingFullyGrownCrop,
                                            boolean isTargetingNetherWart, BlockState targetState) {
        if (isTargetingCrop && targetState.isIn(BlockTags.CROPS)) {
            ...
            return !isTargetingCrop && !isTargetingNetherWart && targetState.isIn(TeadTags.Blocks.SCYTHE_BLOCKS);
        }
    }
}
