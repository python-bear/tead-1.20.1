...
public class BoneBowItem extends BowItem implements Vanishable {
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClient) {
            ...
            int dropChance = random.nextInt(15);

            if (dropChance == 0) {
                Entity skeletonOwner = new SkeletonEntity(EntityType.SKELETON, world);
                persistentProjectileEntity.setOwner(skeletonOwner);
            }
        ...
        world.playSound(null, playerEntity.getX(), playerEntity.getY(), playerEntity.getZ(), TeadSounds.BONE_BOW_SHOOT, SoundCategory.PLAYERS, 1.0f, 1.0f / (world.getRandom().nextFloat() * 0.4f + 1.2f) + f * 0.5f);
