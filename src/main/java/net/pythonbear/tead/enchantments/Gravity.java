...
public class Gravity extends Enchantment {
    public Gravity(Rarity weight, EnchantmentTarget target, EquipmentSlot... slotTypes) {
    ...
    public void onTargetDamaged(LivingEntity user, Entity target, int level) {
        if (!user.getWorld().isClient() && target instanceof LivingEntity) {
            ...
            for (LivingEntity entity : entities) {
                ...
                double strength = (1.0 / (distance + 0.1)) * ((double) level / 3); // Increase pull strength with level
    ...
    protected boolean canAccept(Enchantment other) {
        return super.canAccept(other) && other != TeadEnchantments.SHOCKWAVE;
    }
}
