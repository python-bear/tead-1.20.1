...
public class TeadRecipeBookResults {
    protected static final Identifier TEXTURE = new Identifier("textures/gui/recipe_book.png");
    private final List<TeadAnimatedResultButton> resultButtons = Lists.newArrayListWithCapacity(20);
    @Nullable
    private TeadAnimatedResultButton hoveredResultButton;
    ...
    public TeadRecipeBookResults() {
        for (int i = 0; i < 20; ++i) {
            this.resultButtons.add(new TeadAnimatedResultButton());
        }
    }
    ...
    public void setGui(TeadRecipeBookWidget widget) {
    ...
    private void refreshResultButtons() {
        ...
        for (int j = 0; j < this.resultButtons.size(); ++j) {
            TeadAnimatedResultButton animatedResultButton = this.resultButtons.get(j);
    ...
    public void draw(DrawContext context, int x, int y, int mouseX, int mouseY, float delta) {
        ...
        for (TeadAnimatedResultButton animatedResultButton : this.resultButtons) {
    ...
    public void drawTooltip(DrawContext context, int x, int y) {
        if (this.client.currentScreen != null && this.hoveredResultButton != null && !this.alternatesWidget.isVisible()) {
            context.drawTooltip(this.client.textRenderer, this.hoveredResultButton.getTooltipList(), x, y);
        }
    }
